...
def catalog_dir(survey='main', verspec='guadalupe', version='test', base_dir='/global/cfs/cdirs/desi/survey/catalogs'):
    if args.survey == 'edav1':
        logger.info('Dir:', os.path.join(base_dir, survey, verspec, 'LSScats', version))
        return os.path.join(base_dir, survey, verspec, 'LSScats', version) #Cameron's edit - SV3
    elif args.survey == 'DA02':
        logger.info('Dir:', os.path.join(base_dir, survey, 'LSS', verspec, 'LSScats', version))
        return os.path.join(base_dir, survey, 'LSS', verspec, 'LSScats', version) #for DA0.2
    else:
...
def get_clustering_positions_weights(catalog, distance, zlim=(0., np.inf), region='', maglim=None, weight_type='default', name='data', return_mask=False, option=None):
    catalog.dtype.names
    """if args.survey == 'edav1':
        ABSMAG_R = 'ABSMAG_R'
    elif args.survey == 'DA02':
    ...
    try:
        print(catalog['PHOTSYS'][0])
    except KeyError: 
        # no photsys column
        catalog['PHOTSYS'] = region
        
    if maglim is None:
        mask = (catalog['Z'] >= zlim[0]) & (catalog['Z'] < zlim[1])
    if maglim is not None:
        if args.vol_lim == 'True': 
            print('defining volume-limited sample')
            import sys
            os.environ['CODE_ROOT'] = os.environ['HOME'] + '/code/LSS/py/LSS/DESI_ke'
            # os.environ['CODE_ROOT'] = os.environ['HOME'] + '/DESI'
            sys.path.append(os.environ['HOME'])
            sys.path.append(os.environ['CODE_ROOT'])
            sys.path.append(os.environ['HOME'] + '/code/project')
            
            import vol_lim_samples as vls
            
            mask = vls.vol_lim_sample_2(maglim[1],maglim[0], catalog) #(faintlim, brightlim)
            
            plt.figure() 
            plt.plot(catalog['Z'], catalog['ABSMAG_RP1'], lw=0, marker=',')
            plt.plot(catalog['Z'][mask],catalog['ABSMAG_RP1'][mask],lw=0,marker=',')
            plt.gca().invert_yaxis()
            if maglim[0] != None:
                plt.savefig('figures/{:s}_vol_lim_sample_{:n}_{:n}.png'.format(args.survey, maglim[0], maglim[1]))      
            else:
                plt.savefig('figures/{:s}_vol_lim_sample_{:n}.png'.format(args.survey, maglim[1])) 
                       
        else:
            print('defining absolute magnitude-limited sample:', maglim)
            absmask = (catalog['Z'] >= zlim[0]) & (catalog['Z'] < zlim[1]) & (catalog['ABSMAG_RP1'] >= maglim[0]) & (catalog['ABSMAG_RP1'] < maglim[1])
            mask = absmask
            ...
            plt.gca().invert_yaxis()
            if maglim[0] != None:
                plt.savefig('figures/{:s}_abs_lim_sample_{:n}_{:n}.png'.format(args.survey, maglim[0], maglim[1]))  
            else:
                plt.savefig('figures/{:s}_abs_lim_sample_{:n}.png'.format(args.survey, maglim[1]))     
            plt.close()
        
    if args.colour:
        print('splitting sample by colour:', args.colour)
        catalog['COLOUR_CUT'] = 0.21 - 0.03*catalog['ABSMAG_RP1']
        if args.colour == 'red':
            colour_mask = (catalog['REST_GMR_0P1'] > catalog['COLOUR_CUT'])
        if args.colour == 'blue':
            colour_mask = (catalog['REST_GMR_0P1'] <= catalog['COLOUR_CUT'])
        ...
        plt.figure()
        plt.hist(catalog['REST_GMR_0P1'][mask], bins=np.arange(-0.5,1.5,0.025))
        plt.savefig('figures/{:s}_colour_{:s}.png'.format(args.survey, args.colour))
        plt.close()
...
def read_clustering_positions_weights(distance, zlim =(0., np.inf), maglim =None,weight_type='default', name='data', concatenate=False, option=None, region=None, **kwargs):
    ...
    def read_positions_weights(name):
        ...
        for reg in region:
            ...
            if isscalar:
                cat_fns = [cat_fns]
            positions_weights = [get_clustering_positions_weights(Table.read(cat_fn), distance, zlim=zlim, region=reg, maglim=maglim, weight_type=weight_type, name=name, option=option) for cat_fn in cat_fns]
            if isscalar:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--vol_lim', help='whether to define volume-limited sample', default=False)
    parser.add_argument('--colour', help='whether to split by colour', choices=[False, 'red', 'blue'], default=False)
